...
window.citeParser = function(){
    var data = {};
    var record = CWDataCenter.pubSelected;
    data.title = CwZ(".description h1").text() || atob(record.title);
    data.authors= record.authors.replace(/;/g, ",");

    if(!record.listingPage){
        var elem = CwZ(".abstract .bd .gutter");
        data.abs = elem.find(".paragraph").text();
        data.keywords = elem.find(".keywords");//.html(elem.find(".keywords li").each(function(){this.innerHTML = this.innerHTML.replace(",","")}));
    }
    return data;
};
